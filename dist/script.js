...
/* harmony import */ var _modules_scrollingWithRAF__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ./modules/scrollingWithRAF */ "./src/js/modules/scrollingWithRAF.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_pictureSize__WEBPACK_IMPORTED_MODULE_8__["default"])('.sizes-block'); // accordion('.accordion-heading', '.accordion-block');//css-реализация

  Object(_modules_accordion__WEBPACK_IMPORTED_MODULE_9__["default"])('.accordion-heading'); //js-реализация

  Object(_modules_burger__WEBPACK_IMPORTED_MODULE_10__["default"])('.burger-menu', '.burger'); // scrolling('.pageup');

  Object(_modules_scrollingWithRAF__WEBPACK_IMPORTED_MODULE_12__["default"])('.pageup');
...
var accordion = function accordion(triggersSelector) {
  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      this.nextElementSibling.classList.toggle('active-content'); // содержит ли элемент класс активности
...
var forms = function forms() {
  var form = document.querySelectorAll('form'),
      ...
      upload = document.querySelectorAll('[name="upload"]'); // статусы с изображениями
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // создаем оповещения
      ...
      var formData = new FormData(item); // собираем данные из форм

      var api; //для динамического пути
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {
      ...
      var range = elem.createTextRange(); // для старых браузеров(ручной полифил)
  ...
  function createMask(event) {
    ...
    def = matrix.replace(/\D/g, ''),
        // убираем не цифры
    val = this.value.replace(/\D/g, ''); // если к-во цифр >= кол-ву в value,заменяем на стандартное(+7)
...
var modals = function modals() {
  var btnPressed = false; // следим была ли нажата кнопка
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        if (destroy) {
          item.remove(); // удаляем item, если destroy = true
        }
        ...
        document.body.style.marginRight = "".concat(scroll, "px"); //добавляем margin-rigth
      });
    });
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display; //проверяем отображается ли модальное оокно пользователю в текущий момент времени
  ...
  function calcScroll() {
    ...
    div.remove();
    return scrollWidth;
  } //ф-ция отображния модалки сподарком, если пользователь проскроллил страницу до конце и не нажимал на кнопки
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    var img = block.querySelector('img'); //somethins.png => something-1.png
  ...
  function hideImg(block) {
    var img = block.querySelector('img'); //something-1.png => somethins.png
...
var scrolling = function scrolling(upSelector) {
  ...
  var calcScroll = function calcScroll() {
    upElem.addEventListener('click', function (event) {
      ...
      if (this.hash !== '') {
        // example.com/#up (#up - hash)
        event.preventDefault();
        var hashElement = document.querySelector(this.hash),
            hashElementTop = 0; // перебираем px родительских элементов и узнаём сколько px нужно будет пролистать 
  ...
  var smoothScroll = function smoothScroll(from, to, hash) {
    ...
    var move = setInterval(function () {
      var scrollTop = Math.round(body.scrollTop || element.scrollTop); // если долистали до того момента, который был нужен
...
/***/ "./src/js/modules/scrollingWithRAF.js":
/*!********************************************!*\
  !*** ./src/js/modules/scrollingWithRAF.js ***!
  \********************************************/
/*! exports provided: default */
...
// JS-скролл c requestAnimationFrame()
var scrollingRAF = function scrollingRAF(upSelector) {
  var upElem = document.querySelector(upSelector);
  window.addEventListener('scroll', function () {
    if (document.documentElement.scrollTop > 1650) {
      upElem.classList.add('animated', 'fadeIn');
    ...
    link.addEventListener('click', function (event) {
      ...
      function step(time) {
        ...
        var progress = time - start,
            r = toBlock < 0 ? Math.max(widthTop - progress / speed, widthTop + toBlock) : Math.min(widthTop + progress / speed, widthTop + toBlock); //кол-во px
...
/* harmony default export */ __webpack_exports__["default"] = (scrollingRAF);
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  //     btn.remove();
  // });
  // подгрузка данных с db.json
  // для запуска: npx json-server src/assets/db.json
